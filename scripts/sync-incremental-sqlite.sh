...
if [ "$SYNC_PHASE" = "market" ]; then
    ...
    run_sync "日线增量" $SYNC_CMD src/sync_daily_sqlite.py --incremental --table all || true
    echo "[$(date)] --- ETF 行情增量同步 ---" >> "$LOG_FILE"
    run_sync "ETF增量" $SYNC_CMD src/sync_etf_sqlite.py --incremental || true
...
PRE_DONE=false
PRE_CP=$(/data/jqdata-platform/.venv/bin/python /data/jqdata-platform/scripts/sync_meta_get.py checkpoint:stock_daily_pre)
if [ -z "$PRE_CP" ]; then
    PRE_MAX=$(/data/jqdata-platform/.venv/bin/python scripts/sync_meta_get.py checkpoint:stock_daily_pre)
...
INDEX_DONE=false
INDEX_CP=$(/data/jqdata-platform/.venv/bin/python /data/jqdata-platform/scripts/sync_meta_get.py checkpoint:index_daily)
...
POST_STARTED=false
POST_CP=$(/data/jqdata-platform/.venv/bin/python /data/jqdata-platform/scripts/sync_meta_get.py checkpoint:stock_daily_post)
if [ -n "$POST_CP" ]; then
    POST_STARTED=true
fi
...
if [ "$PRE_DONE" = false ]; then
    ...
    run_sync "pre断点续传" $SYNC_CMD src/sync_daily_sqlite.py --resume --fq pre --table stock || true
else
...
if [ "$INDEX_DONE" = false ]; then
    ...
    run_sync "index断点续传" $SYNC_CMD src/sync_daily_sqlite.py --resume --fq pre --table index || true
else
...
if [ "$POST_STARTED" = true ]; then
    ...
    run_sync "post断点续传" $SYNC_CMD src/sync_daily_sqlite.py --resume --fq post --table stock || true
else
    if [ "$PRE_DONE" = true ] && [ "$INDEX_DONE" = true ]; then
        echo "[$(date)] 首次启动 post 全量同步" >> "$LOG_FILE"
        run_sync "post首次全量" $SYNC_CMD src/sync_daily_sqlite.py --resume --fq post --table stock || true
    else
...
run_sync "ETF日线增量" $SYNC_CMD src/sync_etf_sqlite.py --incremental || true

ETF_DONE=false
ETF_CP=$(/data/jqdata-platform/.venv/bin/python /data/jqdata-platform/scripts/sync_meta_get.py checkpoint:etf_daily)
if [ -n "$ETF_CP" ] && [ "$ETF_CP" = "$TRIAL_END_VAL" ]; then
    ETF_DONE=true
fi
if [ "$ETF_DONE" = false ]; then
    echo "[$(date)] 执行 ETF 断点续传" >> "$LOG_FILE"
    run_sync "ETF断点续传" $SYNC_CMD src/sync_etf_sqlite.py --resume || true
else
...
CAL_DAYS=$(/data/jqdata-platform/.venv/bin/python -c "import sqlite3,datetime; r=sqlite3.connect('/data/jqdata-platform/data/jqdata.db').execute('SELECT max(trade_date) FROM trade_calendar').fetchone(); print((datetime.date.today()-datetime.date.fromisoformat(r[0])).days if r and r[0] else 999)" 2>/dev/null || echo "999")
if [ "$CAL_DAYS" -gt 7 ] 2>/dev/null; then
    echo "[$(date)] trade_calendar 超过7天未更新，执行同步" >> "$LOG_FILE"
    run_sync "交易日历" $SYNC_CMD src/sync_trade_calendar_sqlite.py || true
fi

echo "[$(date)] 执行 index_weights 增量同步" >> "$LOG_FILE"
run_sync "指数权重" env SYNC_MODE=incremental $SYNC_CMD src/sync_index_weights_sqlite.py || true

echo "[$(date)] 执行 stk_xr_xd 增量同步" >> "$LOG_FILE"
run_sync "除权除息" env SYNC_MODE=incremental $SYNC_CMD src/sync_stk_xr_xd_sqlite.py || true

# 前复权历史回刷：stk_xr_xd 之后执行（依赖最新除权数据），修正除权导致的历史前复权价失真
echo "[$(date)] 执行前复权历史回刷(近期除权股票)" >> "$LOG_FILE"
run_sync "前复权回刷" $SYNC_CMD src/sync_daily_sqlite.py --refresh-xr || true

echo "[$(date)] 执行 extended 增量同步" >> "$LOG_FILE"
run_sync "融资融券/龙虎榜" $SYNC_CMD src/sync_extended_sqlite.py --incremental --days 7 || true

echo "[$(date)] 执行 fundamentals 增量同步" >> "$LOG_FILE"
run_sync "估值/财务" $SYNC_CMD src/sync_fundamentals_sqlite.py --incremental --days 7 || true

# 每周日补充同步季度财报数据（避免增量模式跳过季度表）
if [ "$(date +%u)" = "7" ]; then
    echo "[$(date)] 周日补充季度财报同步" >> "$LOG_FILE"
    run_sync "季度财报" $SYNC_CMD src/sync_fundamentals_sqlite.py --quarterly || true
fi

# ── 额度报告 ──
QUOTA_USED=$(/data/jqdata-platform/.venv/bin/python scripts/sync_meta_get.py quota_used_today); QUOTA_USED=${QUOTA_USED:-0}
