...
| 环境 | 服务器 | 配置 | 角色 | 部署内容 |
|------|--------|------|------|---------|
| 本地开发 | 本机 | — | — | `docker compose up` |
| 核心数据 | `101.132.161.52` (D) | 8核 64GB | `root` / 后续建运维用户 | ClickHouse主 + Redis + FastAPI |
| 数据副本 | `139.196.186.67` (C) | 4核 32GB | `root` / 后续建运维用户 | ClickHouse从 + Grafana + Airflow |
| 应用服务 | `106.14.141.212` (A) | 4核 16GB 5Mbps | — | ❌ 不动现有业务 |
| 测试/存储 | `139.196.34.92` (B) | 4核 16GB 492GB | — | 保留现有测试环境，后续可能扩展MinIO |

**网络：** 四台机器同属阿里云 VPC `172.24.52.0/24`，内网互通。数据服务端口仅开放内网访问。
