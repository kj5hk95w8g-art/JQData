#!/usr/bin/env python3
"""JQData -> sqlite 日线数据同步（M6 sqlite 化版，与 sync_daily.py 逻辑等价）

由 ClickHouse+Redis 版改写为写 sqlite（/data/jqdata-platform/data/jqdata.db，
可用 JQDATA_DB 覆盖）。原 sync_daily.py 保留作为回退。

改写点（ClickHouse 特有 → sqlite 等价物，均以注释标注）：
  - ALTER TABLE ... DELETE + _wait_for_mutations（异步 mutation）→ 同步 DELETE + 事务提交
  - %(name)s 参数风格 → ? 占位符（007 规范）
  - system.mutations 轮询（_wait_mutations 模块函数）删除（sqlite 无此概念）
  - stk_xr_xd 的 today() - %(days)s 日期运算改在 Python 层计算后 ? 传值
...
环境变量：
  ...
  JQDATA_DB            sqlite 库文件，默认 /data/jqdata-platform/data/jqdata.db
...
class JQDataSync(SyncBaseSqlite):
    """股票 & 指数日线同步（sqlite 版）"""
    ...
    def sync_security_info(self) -> int:
        ...
        # sqlite：全量覆盖（原 ClickHouse 为 ALTER TABLE DELETE WHERE 1=1 + 等 mutation）
        self.conn.execute("DELETE FROM security_info")
        records = []
        ...
        self.conn.executemany(
            ...
            "(code, display_name, name, sec_type, exchange, start_date, end_date, list_status) "
            f"VALUES ({ph})",
            records,
        )
        self.conn.commit()
    ...
    def _insert_stock_batch(self, table: str, records: List[Tuple]):
        """股票日线专用批量插入（字段固定），委托统一 _insert_batch
    ...
    def sync_pre_refresh_xr(self, days: int = 14) -> int:
        ...
        """
        # 原 ClickHouse：today() - %(days)s 日期运算；sqlite 改在 Python 层计算后 ? 传值
        today = date.today()
        start = (today - timedelta(days=days)).isoformat()
        rows = self.conn.execute(
            "SELECT DISTINCT code FROM stk_xr_xd "
            "WHERE a_xr_date >= ? AND a_xr_date <= ?",
    ...
    def sync_index_daily(
        ...
        self.conn.execute(
            "DELETE FROM index_daily WHERE trade_date >= ? AND trade_date <= ?",
...
def main():
    parser = argparse.ArgumentParser(description="JQData -> sqlite 同步")
