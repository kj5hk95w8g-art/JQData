...
class JQDataSync:
    ...
    def sync_stock_daily(
        ...
        # 先删除本次同步日期范围内的数据，避免重复插入
        self.ch.execute(f"ALTER TABLE {table} DELETE WHERE trade_date BETWEEN '{start_date}' AND '{end_date}'")
        self._wait_for_mutations(table)
        logger.info(f"已清理 {table} {start_date}~{end_date} 的旧数据")
