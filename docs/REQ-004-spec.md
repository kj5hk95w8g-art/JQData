...
**为什么不用明文Token：** 签名每次不同，抓包无法直接复用。

**盐值更换：** 不主动更换，泄露后再换（被动应对）。

### 3.2 入口：弹性公网IP（待定）

| 阶段 | 方案 | 状态 |
|------|------|------|
| **当前** | 公网IP访问（101.132.161.52:18080）| ✅ 已可用 |
| **后期** | 绑定域名 + HTTPS | 待定，后续增加 |
...
| **第二层** | 不公开宣传 | 降低被发现概率 | ✅ 已完成 |
| **第三层** | Nginx单IP限流 | 内部使用暂不配置 | ⏸️ 待定 |
...
| 风险 | 应对方案 |
|------|---------|
| **盐值泄露** | 不主动更换，发现泄露后直接切换新盐值 |
| **时间不同步** | 已去掉时间窗口检查，不存在此问题 |
...
- [ ] ~~单IP超过100条/秒返回 429~~（内部使用暂不配置）
- [ ] ~~盐值更换流程~~（简化版不主动更换，泄露后手动换）
...
| 弹性公网IP | ✅ 已确定 | 101.132.161.52:18080 |
